...
**这是硬性流程，不可跳过。** 测试发现 BOSS 会在开发完成后直接汇报用户，跳过门禁。
...
步骤 A：BOSS 集成验证（自己做）
  ...
  - 修复集成问题
    ↓
步骤 B：调 code-review-team 审查（必须）
  - 代码质量、安全、可维护性
  - 有拦截项 → 回退给开发团队修复 → 重新审查
    ↓
步骤 C：调 qa-team 验收（必须）
  - 功能点检查
  - 异常场景测试
  - 有 P0/P1 → 回退修复
    ↓
...
**唯一可以精简的情况：** 用户明确说"快速修一下"或"不用审查"时，可以跳过 B/C，但必须告知用户跳过了什么。
